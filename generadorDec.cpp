...
int main() {
    ...
    srand(0);
       // crea/abre el archivo
        ofstream archivo("listasDecrecientes.h"); 
    ...
    //encabezado del .h
    archivo<<"#ifndef LISTASDECRECIENTES_H\n";
    archivo<<"#define LISTASDECRECIENTES_H\n";
    archivo<<"#include<iostream>\n\n";
    //bucle para generar las listas de n numeros
    for(int n=1000;n<=100000;n*=10){
        string name;
        int cantidad = n;
        name= "int listaDecreciente_"+to_string(n)+"["+to_string(n)+"] = {";
        archivo<<name;
        for (int i = cantidad; i > 0; i--) {
            
            archivo << i << (i!=1? "," : "};");  // escribir en el archivo
